...
- Velger riktig avtale basert på regler **(OBS! ALLE BESTILLINGER MÅ KUNNE FÅ TREFF PÅ TILORDNINGSSTØTTE! KREVER KONFIGURASJON AV AVTALEMAPPING)** – se [veiledning for å konfigurere Smart-tildeling](Smart-tildeling_Konfigurasjon.md)
...
- Fikser gammel NISSY-bug hvor Reisemåte sporadisk ble satt blank eller til feil verdi ved redigering av bestilling – feltet sjekkes mot bestillingens plakat og rettes automatisk.
...
- Sjåfør-maler fylles ut med turnummer, adresser, hentetid og pasientens initialer når turen er merket i pågående oppdrag – alle bestillinger på turen tas med.
...
6. **Kjør sjekk-scriptene flere ganger om dagen**: Det anbefales å kjøre Sjekk-Bestilling `ALT+2`, Sjekk-Plakat `ALT+3` og Sjekk-Telefon `ALT+4` flere ganger hver dag, både på dagens turer og morgendagens turer. Det kommer inn nye bestillinger hele veien, så én sjekk om morgenen fanger ikke opp alt.
7. **Sjekkene lønner seg**: Jevnlig bruk av sjekk-scriptene avdekker et stort antall bestillinger som er verdt å rette opp i. Det forebygger bomturer og gir bedre datakvalitet på bestillingene.
